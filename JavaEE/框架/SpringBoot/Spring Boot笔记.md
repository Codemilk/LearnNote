...
​		1）、自定义异常处理&返回定制json数据；没有自适应（面对不同的客户端，有不同的反馈）
...
**注意：如果themelyf的异常获取不到,在你的配置文件加入这两条**

![image-20201030183508040](https://raw.githubusercontent.com/Codemilk/LearnNotes/main/Pic/20201030183516.png)

​		2）、转发到/error进行自适应响应效果处理，没法定制异常信息
...
```java
     ...
     *           并在其中调用了ConfigurableWebServerFactory的方法，对servlet容器管理
     *
     */
    @Bean
...
spring:
  datasource:
    url: jdbc:mysql://120.26.176.204:3306/jdbc?useUnicode=true&characterEncoding=utf8&useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true
